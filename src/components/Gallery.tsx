      ...
      <div className="relative mx-auto max-w-6xl">
        ...
        <div className="columns-1 gap-4 sm:columns-2 sm:gap-5 lg:columns-3 lg:gap-6">
          {galleryImages.map((img, index) => (
            ...
            >
              <span 
                className="relative block overflow-hidden rounded-2xl shadow-2xl ring-1 transition duration-500 ease-out group-hover:-translate-y-1 group-hover:shadow-[0_24px_48px_-12px_rgba(0,0,0,0.2)] sm:rounded-[1.35rem]" 
                style={{ backgroundColor: "rgba(212,175,55,0.1)", boxShadow: "0 4px 24px -4px rgba(0,0,0,0.1), inset 0 1px 0 rgba(212,175,55,0.48)" }}
              >
                <span className="relative block w-full overflow-hidden" style={{ aspectRatio: img.aspectRatio }}>
                  <img src={img.src} alt="" className="h-full w-full object-cover transition duration-700 ease-out group-hover:scale-[1.05]" style={{ objectPosition: (img as any).objectPosition || "center" }} loading="lazy" />
                  ...
                  <span className="absolute bottom-3 left-3 right-3 flex items-end justify-between gap-2 sm:bottom-4 sm:left-4 sm:right-4">
                    <span className="text-[10px] font-medium uppercase tracking-[0.35em] font-sans" style={{ color: "#AA8000" }}>
                      View
                    </span>
                    <span className="flex h-9 w-9 items-center justify-center rounded-full backdrop-blur-md transition duration-300 group-hover:scale-110 sm:h-10 sm:w-10" style={{ background: "rgba(212,175,55,0.15)", border: "1px solid rgba(212,175,55,0.48)" }} aria-hidden="true">
